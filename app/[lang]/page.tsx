...
export default function Home({ params }: { params: { lang: Locale } }) {
  ...
  const activeItem =
    activeItemType === "education"
      ? education.find((edu) => edu.id === activeItemId) || null
    ...
    <main className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-12 space-y-4 sm:space-y-0">
          <h1 className="text-4xl font-bold text-gray-800">{String(t("title"))}</h1>
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
            <ResumeActions
              linkedinUrl={String(t("linkedinUrl"))}
              pdfUrl={String(t("pdfUrl"))}
              linkedinText={String(t("linkedinText"))}
              downloadText={String(t("downloadText"))}
            />
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/2">
            <ResumeDetail item={activeItem} selectItemText={String(t("selectCompany"))} />
          </div>
        </div>
        <div className="mt-12">
